...
<div x-data="{ bubbleOpen: false }" class="w-full max-w-[16rem] mx-auto h-full">
    ...
    <div @click="{{ $isAdmin ? '' : 'bubbleOpen = true' }}"
        ...
        <div class="flex flex-col flex-1 px-0.5 rtl:text-right ltr:text-left">
            <span class="text-[8px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest mb-1 block">{{ $category }}</span>
            <h3 class="text-xs md:text-sm font-black text-slate-800 dark:text-white line-clamp-1 mb-1 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{{ $title }}</h3>
            <p class="text-[10px] text-slate-700 dark:text-slate-400 font-medium line-clamp-2 mb-3 leading-relaxed">{{ $description }}</p>

            {{-- Price + Button / Admin Controls --}}
            <div class="flex items-center justify-between mt-auto pt-2 border-t border-slate-100 dark:border-slate-800/40 w-full">
                ...
                @if ($isAdmin)
                    <div class="flex items-center w-full gap-2 px-4">
                        @if (!empty($oldPrice))
                            ...
                            @endif
                            <span class="text-sm font-black text-slate-900 dark:text-white">{{ $price }}</span>
                    </div>
                @else
                    {{-- وضع العرض العادي: السعر وزر الشراء --}}
                    <div class="flex flex-col shrink-0">
                        @if (!empty($oldPrice))
                            <span class="text-[11px] font-bold text-slate-400 line-through leading-none mb-0.5">{{ $oldPrice }}</span>
                        @endif
                        <span class="text-sm font-black text-slate-900 dark:text-white">{{ $price }}</span>
                    </div>
                    <button type="button" @click.stop="window.location.href='{{ $url }}'" class="flex items-center py-1.5 px-3 gap-1.5 bg-slate-900 dark:bg-purple-600 text-white font-bold text-xs rounded-lg hover:scale-105 active:scale-95 transition-all shadow-sm cursor-pointer">
        ...
        <div x-show="bubbleOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto" x-cloak>
            ...
            <div x-show="bubbleOpen" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 scale-75 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-75 translate-y-4" 
                ...
                <div class="flex flex-col gap-4 mt-2">
                    ...
                    <div class="px-1">
                        <span class="text-[9px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest block mb-1">{{ $category }}</span>
                        <h2 class="text-base md:text-lg font-black text-slate-800 dark:text-white mb-2 font-orbitron">{{ $title }}</h2>
                        <p class="text-xs text-slate-800 dark:text-slate-400 font-medium leading-relaxed bg-slate-50 dark:bg-slate-950/40 p-3 rounded-xl border border-slate-100 dark:border-slate-800/40">{{ $description }}</p>
                    </div>
                    <div class="flex items-center justify-between pt-3.5 border-t border-slate-100 dark:border-slate-800/60 mt-1">
                        <div class="flex flex-col">
                            @if (!empty($oldPrice))
                                <span class="text-[14px] font-bold text-slate-600 dark:text-slate-500 line-through mb-0.5">{{ $oldPrice }}</span>
                            @endif
                            <span class="text-lg font-orbitron font-black text-slate-900 dark:text-white tracking-tight">{{ $price }}</span>
                        </div>
